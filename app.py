...
if st.button("🚀 Run Full Design Pipeline"):
    with st.spinner("Analyzing and Optimizing..."):
        ...
        st.session_state.analysis_results = {
            'refined_aa': refined_aa, 'final_pi': final_pi, 'status': status,
            'spec_res': spec_res, 'off_targets': off_targets,
...
if st.session_state.analysis_results:
    ...
    with st.expander("📊 判定基準とスコアの見方について"):
        ...
        | 指標 | Success基準 | 役割 |
        | :--- | :--- | :--- |
        | **等電点 ($pI$)** | **8.5 - 9.5** | 細胞内での溶解性を維持するために必要です。 |
        | **特異性スコア ($\Delta \Delta G$)** | **8.0以上** | 変異型のみを狙い撃ちできているかを示します。 |
        | **構造信頼度 (pLDDT)** | **70以上** | 構造予測が信頼できるレベルかを示します。 |
        | **界面精度 (iPAE)** | **10.0Å以下** | 配置が正確に予測できているかを示します。 |
        """)
    ...
    with btn_col2:
        validator = AFMValidator(f"results/designed.pdb", sequence=res['refined_aa'])
        with st.spinner("Fetching 3D structure..."):
            pdb_data = validator.get_pdb_content()
        st.download_button(label="🧊 Download 3D Model (.pdb)", data=pdb_data, file_name="designed_intrabody.pdb", mime="text/plain")